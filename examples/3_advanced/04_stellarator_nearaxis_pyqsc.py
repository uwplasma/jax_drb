from __future__ import annotations

"""
04_stellarator_nearaxis_pyqsc.py

Purpose
-------
Show how to run the same `jaxdrb` linear workflows on a **stellarator** geometry derived from a
near-axis construction (pyQSC).

This example is inspired by:
  - R. Jorge & M. Landreman, Plasma Phys. Control. Fusion 63 (2021) 014001 (NAQS paper).

What this script does:
  1) Load a near-axis configuration from pyQSC (built-in "from_paper" cases).
  2) Construct a field-line-following mapping x(r, alpha, l) by finite differencing the mapping
     (r, alpha, l) ↦ x in real space.
  3) Compute tabulated field-line coefficients along l:
       - metric (gxx, gxy, gyy) for k_perp^2(l),
       - curvature coefficients (curv_x, curv_y),
       - a parallel-derivative scaling factor dpar_factor,
       - B(l) (for diagnostics/plotting).
  4) Run ky scans for two parameter sets that expose resistive-like vs inertial-like branches.
  5) Compute a toy SOL width estimate via the Halpern fixed-point rule Lp = q (gamma/ky)_max.

Important notes
---------------
- This is a *methodology* demo; the default physics model is simplified and electrostatic.
- The near-axis mapping and coefficient definitions here follow the spirit of the NAQS paper and
  are intended to produce reasonable, smooth 1D coefficients along the field line.
- This script depends on optional packages:
    - pyQSC (import name `qsc`)
    - SciPy (for spline interpolation on the near-axis boundary surfaces)

Run
---
From the repo root, with a pyQSC checkout next to this repo:

  PYTHONPATH=../pyQSC-main python examples/3_advanced/04_stellarator_nearaxis_pyqsc.py

Outputs
-------
Written to `out/3_advanced/stellarator_nearaxis_pyqsc/`.
"""

import json
import os
import sys
from dataclasses import asdict, dataclass
from pathlib import Path

import jax
import numpy as np

ROOT = Path(__file__).resolve().parents[2]
sys.path.insert(0, str(ROOT / "src"))

from jaxdrb.analysis.lp import LpFixedPointResult, solve_lp_fixed_point
from jaxdrb.analysis.plotting import (
    save_eigenfunction_panel,
    save_eigenvalue_spectrum,
    save_geometry_overview,
    save_json,
    save_lp_history,
    save_scan_panels,
    set_mpl_style,
)
from jaxdrb.analysis.scan import Scan1DResult, scan_ky
from jaxdrb.geometry.tabulated import TabulatedGeometry
from jaxdrb.linear.arnoldi import arnoldi_eigs, arnoldi_leading_ritz_vector
from jaxdrb.linear.matvec import linear_matvec
from jaxdrb.models.cold_ion_drb import State, equilibrium
from jaxdrb.models.params import DRBParams


def _require_pyqsc():
    try:
        from qsc import Qsc  # type: ignore

        return Qsc
    except Exception as exc:  # pragma: no cover
        raise SystemExit(
            "This example requires pyQSC.\n\n"
            "If you have a local checkout next to this repo, run:\n"
            "  PYTHONPATH=../pyQSC-main python examples/3_advanced/04_stellarator_nearaxis_pyqsc.py\n\n"
            "Otherwise install pyQSC (import name is typically `qsc`) and try again.\n\n"
            f"Import error: {exc}"
        ) from exc


def _require_scipy():
    try:
        from scipy.interpolate import RectBivariateSpline  # type: ignore

        return RectBivariateSpline
    except Exception as exc:  # pragma: no cover
        raise SystemExit(
            "This example requires SciPy for spline interpolation.\n\n"
            "Install SciPy and try again.\n\n"
            f"Import error: {exc}"
        ) from exc


def _central_diff_periodic(x: np.ndarray, dx: float) -> np.ndarray:
    return (np.roll(x, -1, axis=0) - np.roll(x, 1, axis=0)) / (2.0 * dx)


@dataclass
class NearAxisGeomConfig:
    qsc_config: str = "r1 section 5.1"
    r: float = 0.1
    alpha: float = 0.0
    nl: int = 256
    dr: float = 1e-3
    dalpha: float = 2e-3
    max_turns: int = 24
    turns: int | None = None


def build_tabulated_geometry_from_pyqsc(
    cfg: NearAxisGeomConfig, out_npz: Path
) -> TabulatedGeometry:
    """Build a TabulatedGeometry by finite-differencing the near-axis mapping from pyQSC."""

    Qsc = _require_pyqsc()
    RectBivariateSpline = _require_scipy()

    qsc = Qsc.from_paper(cfg.qsc_config)

    iota = float(qsc.iota)
    if cfg.turns is None:
        # Choose a number of toroidal turns that approximately closes the field line:
        # want iota * turns ~ integer.
        best_turns = None
        for turns in range(1, int(cfg.max_turns) + 1):
            mismatch = abs(iota * turns - round(iota * turns))
            cand = (mismatch, turns)
            if best_turns is None or cand < best_turns:
                best_turns = cand
        assert best_turns is not None
        turns = int(best_turns[1])
    else:
        turns = int(cfg.turns)

    L = 2.0 * np.pi * turns
    l = np.linspace(0.0, L, cfg.nl, endpoint=False)
    dl = float(l[1] - l[0])

    # Build a smooth mapping along the field line using pyQSC's nu spline.
    # - Use phi modulo 2π (pyQSC periodicity).
    # - Use varphi for the field-line label used in QSC.
    phi = np.mod(l, 2.0 * np.pi)
    varphi = l + qsc.nu_spline(phi)

    # Field line: theta = alpha + iota * varphi.
    theta0 = cfg.alpha + qsc.iota * varphi
    theta_p = theta0 + cfg.dalpha
    theta_m = theta0 - cfg.dalpha

    def surface_splines(r: float):
        x2d, y2d, z2d, _R2d = qsc.get_boundary(r=r, ntheta=96, nphi=144, ntheta_fourier=24)

        # Periodically extend the grids for robust evaluation near endpoints.
        x2d = np.concatenate([x2d, x2d[:1, :]], axis=0)
        y2d = np.concatenate([y2d, y2d[:1, :]], axis=0)
        z2d = np.concatenate([z2d, z2d[:1, :]], axis=0)
        x2d = np.concatenate([x2d, x2d[:, :1]], axis=1)
        y2d = np.concatenate([y2d, y2d[:, :1]], axis=1)
        z2d = np.concatenate([z2d, z2d[:, :1]], axis=1)

        theta_grid = np.linspace(0.0, 2.0 * np.pi, x2d.shape[0], endpoint=True)
        phi_grid = np.linspace(0.0, 2.0 * np.pi, x2d.shape[1], endpoint=True)

        # Note: RectBivariateSpline expects increasing 1D grids; we use (phi, theta) order for ev().
        sx = RectBivariateSpline(phi_grid, theta_grid, x2d.T)
        sy = RectBivariateSpline(phi_grid, theta_grid, y2d.T)
        sz = RectBivariateSpline(phi_grid, theta_grid, z2d.T)
        return sx, sy, sz

    sx0, sy0, sz0 = surface_splines(cfg.r)
    sxp, syp, szp = surface_splines(cfg.r + cfg.dr)
    sxm, sym, szm = surface_splines(cfg.r - cfg.dr)

    def eval_xyz(sx, sy, sz, theta: np.ndarray) -> np.ndarray:
        th = np.mod(theta, 2.0 * np.pi)
        X = sx.ev(phi, th)
        Y = sy.ev(phi, th)
        Z = sz.ev(phi, th)
        return np.stack([X, Y, Z], axis=1)

    x0 = eval_xyz(sx0, sy0, sz0, theta0)
    x_rp = eval_xyz(sxp, syp, szp, theta0)
    x_rm = eval_xyz(sxm, sym, szm, theta0)
    x_ap = eval_xyz(sx0, sy0, sz0, theta_p)
    x_am = eval_xyz(sx0, sy0, sz0, theta_m)

    # Tangent vectors of the (r, alpha, l) mapping.
    e_r = (x_rp - x_rm) / (2.0 * cfg.dr)
    e_a = (x_ap - x_am) / (2.0 * cfg.dalpha)
    e_l = _central_diff_periodic(x0, dl)

    # Jacobian and covariant basis -> gradient vectors.
    J = np.einsum("ij,ij->i", e_r, np.cross(e_a, e_l))
    grad_r = np.cross(e_a, e_l) / J[:, None]
    grad_a = np.cross(e_l, e_r) / J[:, None]
    grad_l = np.cross(e_r, e_a) / J[:, None]

    # Field-aligned metric for k_perp^2(l) = kx^2 gxx + 2 kx ky gxy + ky^2 gyy.
    gxx = np.einsum("ij,ij->i", grad_r, grad_r)
    gxy = np.einsum("ij,ij->i", grad_r, grad_a)
    gyy = np.einsum("ij,ij->i", grad_a, grad_a)

    # Magnetic field magnitude along the field line (pyQSC provides B_mag in Boozer coords).
    def Bmag(r: float, theta: np.ndarray) -> np.ndarray:
        return np.asarray(qsc.B_mag(r, theta, varphi, Boozer_toroidal=True), dtype=float)

    B0 = Bmag(cfg.r, theta0)
    dB_dr = (Bmag(cfg.r + cfg.dr, theta0) - Bmag(cfg.r - cfg.dr, theta0)) / (2.0 * cfg.dr)
    dB_da = (Bmag(cfg.r, theta_p) - Bmag(cfg.r, theta_m)) / (2.0 * cfg.dalpha)
    dB_dl = _central_diff_periodic(B0, dl)

    gradB = dB_dr[:, None] * grad_r + dB_da[:, None] * grad_a + dB_dl[:, None] * grad_l

    # b × ∇B term (used for a simple curvature proxy).
    b = e_l / np.linalg.norm(e_l, axis=1)[:, None]
    b_x_gradB = np.cross(b, gradB)
    curv_x = 2.0 * np.einsum("ij,ij->i", b_x_gradB, grad_r) / (B0**2)
    curv_y = 2.0 * np.einsum("ij,ij->i", b_x_gradB, grad_a) / (B0**2)

    # d/dl scaling: treat l as an arclength-like coordinate (periodic FD).
    dpar_factor = 1.0 / np.linalg.norm(e_l, axis=1)

    meta = {
        "qsc_config": cfg.qsc_config,
        "r": cfg.r,
        "alpha": cfg.alpha,
        "nl": cfg.nl,
        "dr": cfg.dr,
        "dalpha": cfg.dalpha,
        "turns": turns,
        "iota": iota,
        "L": L,
    }

    np.savez(
        out_npz,
        l=l,
        gxx=gxx,
        gxy=gxy,
        gyy=gyy,
        curv_x=curv_x,
        curv_y=curv_y,
        dpar_factor=dpar_factor,
        B=B0,
        meta=json.dumps(meta, indent=2, sort_keys=True),
    )

    return TabulatedGeometry.from_npz(out_npz)


def _ky_star(scan: Scan1DResult) -> float:
    ratio = np.maximum(scan.gamma_eigs, 0.0) / scan.ky
    return float(scan.ky[int(np.argmax(ratio))])


def _save_scan(out_dir: Path, name: str, scan: Scan1DResult) -> None:
    np.savez(
        out_dir / f"{name}.npz",
        ky=scan.ky,
        gamma_eigs=scan.gamma_eigs,
        omega_eigs=scan.omega_eigs,
        gamma_iv=scan.gamma_iv if scan.gamma_iv is not None else np.nan,
        omega_iv=scan.omega_iv if scan.omega_iv is not None else np.nan,
        eigs=scan.eigs,
    )


def _save_lp(out_dir: Path, name: str, res: LpFixedPointResult) -> None:
    np.savez(out_dir / f"{name}.npz", **asdict(res))


def main() -> None:
    out_dir = Path("out/3_advanced/stellarator_nearaxis_pyqsc")
    out_dir.mkdir(parents=True, exist_ok=True)

    os.environ.setdefault("MPLBACKEND", "Agg")
    os.environ.setdefault("MPLCONFIGDIR", str(out_dir / ".mplcache"))
    set_mpl_style()

    cfg = NearAxisGeomConfig(qsc_config="r1 section 5.1", r=0.1, alpha=0.0, nl=224)
    geom_npz = out_dir / "pyqsc_geom.npz"

    print("Building tabulated geometry from pyQSC…", flush=True)
    geom = build_tabulated_geometry_from_pyqsc(cfg, geom_npz)

    # --- Parameter choices (dimensionless / qualitative) ---
    # These are not tuned to match a specific experimental discharge; they illustrate the workflow.
    params_resistive = DRBParams(
        omega_n=0.6,
        omega_Te=0.0,
        eta=2.0,
        me_hat=2e-3,  # small inertia -> resistive-like branch
        curvature_on=True,
        Dn=0.02,
        DOmega=0.02,
        DTe=0.02,
        kperp2_min=1e-6,
    )
    params_inertial = DRBParams(
        omega_n=0.6,
        omega_Te=0.0,
        eta=0.05,
        me_hat=0.5,  # larger inertia -> inertial-like branch
        curvature_on=True,
        Dn=0.02,
        DOmega=0.02,
        DTe=0.02,
        kperp2_min=1e-6,
    )

    ky = np.linspace(0.03, 0.20, 18)
    kx = 0.0

    print("\nScanning ky (resistive-like)…", flush=True)
    scan_res = scan_ky(
        params_resistive,
        geom,
        ky=ky,
        kx=kx,
        arnoldi_m=32,
        arnoldi_tol=2e-3,
        nev=6,
        do_initial_value=False,
        verbose=True,
        print_every=2,
        seed=0,
    )

    print("\nScanning ky (inertial-like)…", flush=True)
    scan_in = scan_ky(
        params_inertial,
        geom,
        ky=ky,
        kx=kx,
        arnoldi_m=32,
        arnoldi_tol=2e-3,
        nev=6,
        do_initial_value=False,
        verbose=True,
        print_every=2,
        seed=1,
    )

    ky_star_res = _ky_star(scan_res)
    ky_star_in = _ky_star(scan_in)

    save_scan_panels(
        out_dir,
        ky=scan_res.ky,
        gamma=scan_res.gamma_eigs,
        omega=scan_res.omega_eigs,
        title="Near-axis stellarator: resistive-like branch",
        filename="scan_panel_resistive.png",
    )
    save_scan_panels(
        out_dir,
        ky=scan_in.ky,
        gamma=scan_in.gamma_eigs,
        omega=scan_in.omega_eigs,
        title="Near-axis stellarator: inertial-like branch",
        filename="scan_panel_inertial.png",
    )

    # Geometry diagnostics at ky* (kx=0).
    save_geometry_overview(
        out_dir, geom=geom, kx=kx, ky=ky_star_res, filename="geometry_overview.png"
    )

    # Eigenfunctions + spectra at ky* for each branch.
    nl = int(getattr(geom, "l").size)
    y_eq = equilibrium(nl)
    v0 = State.random(jax.random.PRNGKey(0), nl, amplitude=1e-3)

    for name, params, ky_star in [
        ("resistive", params_resistive, ky_star_res),
        ("inertial", params_inertial, ky_star_in),
    ]:
        print(f"\nComputing eigenfunction + spectrum at ky* ({name})…", flush=True)
        matvec = linear_matvec(y_eq, params, geom, kx=kx, ky=ky_star)
        ritz = arnoldi_leading_ritz_vector(matvec, v0, m=120, seed=0)
        save_eigenfunction_panel(
            out_dir,
            geom=geom,
            state=ritz.vector,
            eigenvalue=ritz.eigenvalue,
            kx=kx,
            ky=ky_star,
            kperp2_min=params.kperp2_min,
            filename=f"eigenfunctions_{name}.png",
        )
        arn = arnoldi_eigs(matvec, v0, m=120, nev=120, seed=0)
        save_eigenvalue_spectrum(
            out_dir,
            eigenvalues=arn.eigenvalues,
            highlight=ritz.eigenvalue,
            filename=f"spectrum_{name}.png",
        )

    # --- SOL width estimate via fixed-point rule (toy, for workflow) ---
    print("\nEstimating Lp via fixed-point rule (Halpern 2013 workflow)…", flush=True)
    lp = solve_lp_fixed_point(
        params_resistive,
        geom,
        q=3.0,
        ky=ky,
        Lp0=20.0,
        omega_n_scale=1.0,
        relax=0.6,
        max_iter=12,
        arnoldi_m=30,
        arnoldi_tol=2e-3,
        nev=4,
        seed=2,
        verbose=True,
    )
    save_lp_history(out_dir, history=lp.history, q=3.0, filename="lp_fixed_point_history.png")

    _save_scan(out_dir, "scan_resistive", scan_res)
    _save_scan(out_dir, "scan_inertial", scan_in)
    _save_lp(out_dir, "lp_fixed_point", lp)

    save_json(
        out_dir / "run_meta.json",
        {
            "near_axis_geom": asdict(cfg),
            "params_resistive": params_resistive.__dict__,
            "params_inertial": params_inertial.__dict__,
            "ky_star": {"resistive": ky_star_res, "inertial": ky_star_in},
            "Lp_fixed_point": {
                "Lp": lp.Lp,
                "ky_star": lp.ky_star,
                "gamma_over_ky_star": lp.gamma_over_ky_star,
            },
            "geom_file": str(geom_npz),
        },
    )

    print(f"\nWrote {out_dir}", flush=True)


if __name__ == "__main__":
    main()
