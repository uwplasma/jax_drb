...
class HW2DModel(eqx.Module):
    ...
    def rhs(self, t: float, y: HW2DState) -> HW2DState:
        ...
        drive_n = -self.params.kappa * ddy(phi, self.grid.ky)
        drive_w = -self.params.kappa * ddy(n, self.grid.ky)

        # Resistive/adiabatic coupling.
        couple = self.params.alpha * (phi - n)

        dn = -adv_n + drive_n + couple + self.params.Dn * laplacian(n, self.grid.k2)
        dw = -adv_w + drive_w + couple + self.params.DOmega * laplacian(omega, self.grid.k2)
    ...
    def diagnostics(self, y: HW2DState) -> dict[str, jnp.ndarray]:
        ...
        gradphi_x = jnp.gradient(phi, self.grid.dx, axis=0)
        gradphi_y = jnp.gradient(phi, self.grid.dy, axis=1)
